...
Dialog {
    ...
    ColumnLayout {
        ...
        Label {
            text: dialog.isSmartMode ?
                  "Pick one file from the session — the loader auto-discovers all siblings.\nSupports: .nid (Nanosurf), .mtrx / .I(V)_mtrx (Omicron Matrix), .ps-ppt / .tiff (Park AFM), .wip (WITec)" :
                  dialog.isFolderMode ?
        ...
        Label {
            text: "Supported formats: .nid (Nanosurf), .txt (NeaSpec), .I(V)_mtrx .Aux2(V)_mtrx .Z_flat .I_flat (Omicron Matrix), .ps-ppt .tiff (Park AFM), .wip (WITec)"
            font.pixelSize: 10
            color: textMuted
            Layout.fillWidth: true
            wrapMode: Text.WordWrap
    ...
    FileDialog {
        ...
        nameFilters: [
            "All Measurement Files (*.nid *.txt *.I(V)_mtrx *.Z_flat *.I_flat *.ps-ppt *.tiff *.wip)",
            "Nanosurf Files (*.nid)",
            "NeaSpec Files (*.txt)",
    ...
    FileDialog {
        ...
        nameFilters: [
            "Smart Import Files (*.nid *.mtrx *.I(V)_mtrx *.Aux2(V)_mtrx *.ps-ppt *.tiff *.wip)",
            "Nanosurf Files (*.nid)",
